...
(function($) {
	...
	$.fn.nivoSlider = function(options) {
		...
		function nivoRun(slider, kids, settings, nudge){
			...
			//Run effects"sliceDownRight","sliceDownLeft","sliceUpRight","sliceUpLeft","sliceUpDown","sliceUpDownLeft",
			if(settings.effect == 'random'){
				var anims = new Array("fold");
				vars.randAnim = anims[Math.floor(Math.random()*(anims.length + 1))];
				if (vars.randAnim == undefined) vars.randAnim = 'fold';
			}
			vars.running = true;
...
//			} 
			//			else 
                if(settings.effect == 'fold' || vars.randAnim == 'fold'){
				var timeBuff = 0;
				var i = 0;
				$('.nivo-slice', slider).each(function(){
					var slice = $(this);
